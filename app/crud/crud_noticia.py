...
from app.models.noticia import Noticia # Use model_name_str for consistency
...
def get(db: Session, id: str) -> Optional[Noticia]:
    return db.query(Noticia).filter(Noticia.noticia_id == id).first()

def get_multi(db: Session, *, skip: int = 0, limit: int = 100) -> List[Noticia]:
    return db.query(Noticia).offset(skip).limit(limit).all()

def create(db: Session, *, obj_in: NoticiaCreate) -> Noticia:
    db_obj = Noticia(**obj_in.model_dump())  # Pydantic v2
    db.add(db_obj)
    db.commit()
    db.refresh(db_obj)
    return db_obj
...
def update(
    ...
    *,
    db_obj: Noticia,
    obj_in: Union[NoticiaUpdate, Dict[str, Any]]
) -> Noticia:
...
def remove(db: Session, *, id: str) -> Optional[Noticia]:
    ...
    # The current context is an f-string being written to a file, so Noticia is correct there.
    ...
    obj = db.query(Noticia).filter(Noticia.noticia_id == id).first()
    # A more robust get for single PKs would be:
    # obj = db.get(Noticia, id) # if pk_type_hint is guaranteed to match actual PK type
