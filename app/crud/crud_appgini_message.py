...
from app.models.appgini_message import AppginiMessage # Use model_name_str for consistency
from app.schemas.appgini_message import AppginiMessageCreate, AppginiMessageUpdate

# Note: get_multi might need more sophisticated filtering/ordering in a real app

def get(db: Session, id: int) -> Optional[AppginiMessage]:
    return db.query(AppginiMessage).filter(AppginiMessage.id == id).first()

def get_multi(db: Session, *, skip: int = 0, limit: int = 100) -> List[AppginiMessage]:
    return db.query(AppginiMessage).offset(skip).limit(limit).all()

def create(db: Session, *, obj_in: AppginiMessageCreate) -> AppginiMessage:
    db_obj = AppginiMessage(**obj_in.model_dump())  # Pydantic v2
    db.add(db_obj)
    db.commit()
    db.refresh(db_obj)
    return db_obj
...
def update(
    db: Session,
    *,
    db_obj: AppginiMessage,
    obj_in: Union[AppginiMessageUpdate, Dict[str, Any]]
) -> AppginiMessage:
...
def remove(db: Session, *, id: int) -> Optional[AppginiMessage]:
    ...
    # The current context is an f-string being written to a file, so AppginiMessage is correct there.
    ...
    obj = db.query(AppginiMessage).filter(AppginiMessage.id == id).first()
    # A more robust get for single PKs would be:
    # obj = db.get(AppginiMessage, id) # if pk_type_hint is guaranteed to match actual PK type
