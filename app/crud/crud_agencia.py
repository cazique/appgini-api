...
from app.models.agencia import Agencia # Use model_name_str for consistency
...
def get(db: Session, id: str) -> Optional[Agencia]:
    return db.query(Agencia).filter(Agencia.agencia_id == id).first()

def get_multi(db: Session, *, skip: int = 0, limit: int = 100) -> List[Agencia]:
    return db.query(Agencia).offset(skip).limit(limit).all()

def create(db: Session, *, obj_in: AgenciaCreate) -> Agencia:
    db_obj = Agencia(**obj_in.model_dump())  # Pydantic v2
    db.add(db_obj)
    db.commit()
    db.refresh(db_obj)
    return db_obj
...
def update(
    ...
    *,
    db_obj: Agencia,
    obj_in: Union[AgenciaUpdate, Dict[str, Any]]
) -> Agencia:
...
def remove(db: Session, *, id: str) -> Optional[Agencia]:
    ...
    # The current context is an f-string being written to a file, so Agencia is correct there.
    ...
    obj = db.query(Agencia).filter(Agencia.agencia_id == id).first()
    # A more robust get for single PKs would be:
    # obj = db.get(Agencia, id) # if pk_type_hint is guaranteed to match actual PK type
