...
from app.models.actividad import Actividad # Use model_name_str for consistency
...
def get(db: Session, id: str) -> Optional[Actividad]:
    return db.query(Actividad).filter(Actividad.actividad_id == id).first()

def get_multi(db: Session, *, skip: int = 0, limit: int = 100) -> List[Actividad]:
    return db.query(Actividad).offset(skip).limit(limit).all()

def create(db: Session, *, obj_in: ActividadCreate) -> Actividad:
    db_obj = Actividad(**obj_in.model_dump())  # Pydantic v2
    db.add(db_obj)
    db.commit()
    db.refresh(db_obj)
    return db_obj
...
def update(
    ...
    *,
    db_obj: Actividad,
    obj_in: Union[ActividadUpdate, Dict[str, Any]]
) -> Actividad:
...
def remove(db: Session, *, id: str) -> Optional[Actividad]:
    ...
    # The current context is an f-string being written to a file, so Actividad is correct there.
    ...
    obj = db.query(Actividad).filter(Actividad.actividad_id == id).first()
    # A more robust get for single PKs would be:
    # obj = db.get(Actividad, id) # if pk_type_hint is guaranteed to match actual PK type
