...
from app.models.cliente import Cliente # Use model_name_str for consistency
from app.schemas.cliente import ClienteCreate, ClienteUpdate

# Note: get_multi might need more sophisticated filtering/ordering in a real app

def get(db: Session, id: str) -> Optional[Cliente]:
    return db.query(Cliente).filter(Cliente.cliente_id == id).first()

def get_multi(db: Session, *, skip: int = 0, limit: int = 100) -> List[Cliente]:
    return db.query(Cliente).offset(skip).limit(limit).all()

def create(db: Session, *, obj_in: ClienteCreate) -> Cliente:
    db_obj = Cliente(**obj_in.model_dump())  # Pydantic v2
    db.add(db_obj)
    db.commit()
    db.refresh(db_obj)
    return db_obj
...
def remove(db: Session, *, id: str) -> Optional[Cliente]:
    ...
    # The current context is an f-string being written to a file, so Cliente is correct there.
    ...
    obj = db.query(Cliente).filter(Cliente.cliente_id == id).first()
    # A more robust get for single PKs would be:
    # obj = db.get(Cliente, id) # if pk_type_hint is guaranteed to match actual PK type
