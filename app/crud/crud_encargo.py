...
from app.models.encargo import Encargo # Use model_name_str for consistency
from app.schemas.encargo import EncargoCreate, EncargoUpdate

# Note: get_multi might need more sophisticated filtering/ordering in a real app

def get(db: Session, id: str) -> Optional[Encargo]:
    return db.query(Encargo).filter(Encargo.encargo_id == id).first()

def get_multi(db: Session, *, skip: int = 0, limit: int = 100) -> List[Encargo]:
    return db.query(Encargo).offset(skip).limit(limit).all()

def create(db: Session, *, obj_in: EncargoCreate) -> Encargo:
    db_obj = Encargo(**obj_in.model_dump())  # Pydantic v2
    db.add(db_obj)
    db.commit()
    db.refresh(db_obj)
    return db_obj
...
def update(
    ...
    *,
    db_obj: Encargo,
    obj_in: Union[EncargoUpdate, Dict[str, Any]]
) -> Encargo:
...
def remove(db: Session, *, id: str) -> Optional[Encargo]:
    ...
    # The current context is an f-string being written to a file, so Encargo is correct there.
    ...
    obj = db.query(Encargo).filter(Encargo.encargo_id == id).first()
    # A more robust get for single PKs would be:
    # obj = db.get(Encargo, id) # if pk_type_hint is guaranteed to match actual PK type
