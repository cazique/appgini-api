...
from app.models.estado_encargo import EstadoEncargo # Use model_name_str for consistency
from app.schemas.estado_encargo import EstadoEncargoCreate, EstadoEncargoUpdate

# Note: get_multi might need more sophisticated filtering/ordering in a real app

def get(db: Session, id: int) -> Optional[EstadoEncargo]:
    return db.query(EstadoEncargo).filter(EstadoEncargo.estado_id == id).first()

def get_multi(db: Session, *, skip: int = 0, limit: int = 100) -> List[EstadoEncargo]:
    return db.query(EstadoEncargo).offset(skip).limit(limit).all()

def create(db: Session, *, obj_in: EstadoEncargoCreate) -> EstadoEncargo:
    db_obj = EstadoEncargo(**obj_in.model_dump())  # Pydantic v2
    db.add(db_obj)
    db.commit()
    db.refresh(db_obj)
    return db_obj
...
def update(
    db: Session,
    *,
    db_obj: EstadoEncargo,
    obj_in: Union[EstadoEncargoUpdate, Dict[str, Any]]
) -> EstadoEncargo:
...
def remove(db: Session, *, id: int) -> Optional[EstadoEncargo]:
    ...
    # The current context is an f-string being written to a file, so EstadoEncargo is correct there.
    ...
    obj = db.query(EstadoEncargo).filter(EstadoEncargo.estado_id == id).first()
    # A more robust get for single PKs would be:
    # obj = db.get(EstadoEncargo, id) # if pk_type_hint is guaranteed to match actual PK type
