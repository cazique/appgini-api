...
from app.models.estado_contacto import EstadoContacto # Use model_name_str for consistency
from app.schemas.estado_contacto import EstadoContactoCreate, EstadoContactoUpdate

# Note: get_multi might need more sophisticated filtering/ordering in a real app

def get(db: Session, id: int) -> Optional[EstadoContacto]:
    return db.query(EstadoContacto).filter(EstadoContacto.estado_contacto_id == id).first()

def get_multi(db: Session, *, skip: int = 0, limit: int = 100) -> List[EstadoContacto]:
    return db.query(EstadoContacto).offset(skip).limit(limit).all()

def create(db: Session, *, obj_in: EstadoContactoCreate) -> EstadoContacto:
    db_obj = EstadoContacto(**obj_in.model_dump())  # Pydantic v2
    db.add(db_obj)
    db.commit()
    db.refresh(db_obj)
    return db_obj
...
def update(
    db: Session,
    *,
    db_obj: EstadoContacto,
    obj_in: Union[EstadoContactoUpdate, Dict[str, Any]]
) -> EstadoContacto:
...
def remove(db: Session, *, id: int) -> Optional[EstadoContacto]:
    ...
    # The current context is an f-string being written to a file, so EstadoContacto is correct there.
    ...
    obj = db.query(EstadoContacto).filter(EstadoContacto.estado_contacto_id == id).first()
    # A more robust get for single PKs would be:
    # obj = db.get(EstadoContacto, id) # if pk_type_hint is guaranteed to match actual PK type
