...
from app.models.inmueble import Inmueble # Use model_name_str for consistency
...
def get(db: Session, id: str) -> Optional[Inmueble]:
    return db.query(Inmueble).filter(Inmueble.inmueble_id == id).first()

def get_multi(db: Session, *, skip: int = 0, limit: int = 100) -> List[Inmueble]:
    return db.query(Inmueble).offset(skip).limit(limit).all()

def create(db: Session, *, obj_in: InmuebleCreate) -> Inmueble:
    db_obj = Inmueble(**obj_in.model_dump())  # Pydantic v2
    db.add(db_obj)
    db.commit()
    db.refresh(db_obj)
    return db_obj
...
def update(
    ...
    *,
    db_obj: Inmueble,
    obj_in: Union[InmuebleUpdate, Dict[str, Any]]
) -> Inmueble:
...
def remove(db: Session, *, id: str) -> Optional[Inmueble]:
    ...
    # The current context is an f-string being written to a file, so Inmueble is correct there.
    ...
    obj = db.query(Inmueble).filter(Inmueble.inmueble_id == id).first()
    # A more robust get for single PKs would be:
    # obj = db.get(Inmueble, id) # if pk_type_hint is guaranteed to match actual PK type
