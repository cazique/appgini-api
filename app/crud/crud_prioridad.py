...
from app.models.prioridad import Prioridad # Use model_name_str for consistency
from app.schemas.prioridad import PrioridadCreate, PrioridadUpdate

# Note: get_multi might need more sophisticated filtering/ordering in a real app

def get(db: Session, id: int) -> Optional[Prioridad]:
    return db.query(Prioridad).filter(Prioridad.prioridad_id == id).first()

def get_multi(db: Session, *, skip: int = 0, limit: int = 100) -> List[Prioridad]:
    return db.query(Prioridad).offset(skip).limit(limit).all()

def create(db: Session, *, obj_in: PrioridadCreate) -> Prioridad:
    db_obj = Prioridad(**obj_in.model_dump())  # Pydantic v2
    db.add(db_obj)
    db.commit()
    db.refresh(db_obj)
    return db_obj
...
def update(
    ...
    *,
    db_obj: Prioridad,
    obj_in: Union[PrioridadUpdate, Dict[str, Any]]
) -> Prioridad:
...
def remove(db: Session, *, id: int) -> Optional[Prioridad]:
    ...
    # The current context is an f-string being written to a file, so Prioridad is correct there.
    ...
    obj = db.query(Prioridad).filter(Prioridad.prioridad_id == id).first()
    # A more robust get for single PKs would be:
    # obj = db.get(Prioridad, id) # if pk_type_hint is guaranteed to match actual PK type
