...
from app.models.membership_user import MembershipUser # Use model_name_str for consistency
from app.schemas.membership_user import MembershipUserCreate, MembershipUserUpdate

# Note: get_multi might need more sophisticated filtering/ordering in a real app

def get(db: Session, id: str) -> Optional[MembershipUser]:
    return db.query(MembershipUser).filter(MembershipUser.memberID == id).first()

def get_multi(db: Session, *, skip: int = 0, limit: int = 100) -> List[MembershipUser]:
    return db.query(MembershipUser).offset(skip).limit(limit).all()

def create(db: Session, *, obj_in: MembershipUserCreate) -> MembershipUser:
    db_obj = MembershipUser(**obj_in.model_dump())  # Pydantic v2
    db.add(db_obj)
    db.commit()
    db.refresh(db_obj)
    return db_obj
...
def update(
    db: Session,
    *,
    db_obj: MembershipUser,
    obj_in: Union[MembershipUserUpdate, Dict[str, Any]]
) -> MembershipUser:
...
def remove(db: Session, *, id: str) -> Optional[MembershipUser]:
    ...
    # The current context is an f-string being written to a file, so MembershipUser is correct there.
    ...
    obj = db.query(MembershipUser).filter(MembershipUser.memberID == id).first()
    # A more robust get for single PKs would be:
    # obj = db.get(MembershipUser, id) # if pk_type_hint is guaranteed to match actual PK type
