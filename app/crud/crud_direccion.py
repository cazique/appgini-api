...
from app.models.direccion import Direccion # Use model_name_str for consistency
...
def get(db: Session, id: int) -> Optional[Direccion]:
    return db.query(Direccion).filter(Direccion.direccion_id == id).first()

def get_multi(db: Session, *, skip: int = 0, limit: int = 100) -> List[Direccion]:
    return db.query(Direccion).offset(skip).limit(limit).all()

def create(db: Session, *, obj_in: DireccionCreate) -> Direccion:
    db_obj = Direccion(**obj_in.model_dump())  # Pydantic v2
    db.add(db_obj)
    db.commit()
    db.refresh(db_obj)
    return db_obj
...
def update(
    ...
    *,
    db_obj: Direccion,
    obj_in: Union[DireccionUpdate, Dict[str, Any]]
) -> Direccion:
...
def remove(db: Session, *, id: int) -> Optional[Direccion]:
    ...
    # The current context is an f-string being written to a file, so Direccion is correct there.
    ...
    obj = db.query(Direccion).filter(Direccion.direccion_id == id).first()
    # A more robust get for single PKs would be:
    # obj = db.get(Direccion, id) # if pk_type_hint is guaranteed to match actual PK type
