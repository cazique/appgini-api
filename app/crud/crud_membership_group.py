...
from app.models.membership_group import MembershipGroup # Use model_name_str for consistency
from app.schemas.membership_group import MembershipGroupCreate, MembershipGroupUpdate

# Note: get_multi might need more sophisticated filtering/ordering in a real app

def get(db: Session, id: int) -> Optional[MembershipGroup]:
    return db.query(MembershipGroup).filter(MembershipGroup.groupID == id).first()

def get_multi(db: Session, *, skip: int = 0, limit: int = 100) -> List[MembershipGroup]:
    return db.query(MembershipGroup).offset(skip).limit(limit).all()

def create(db: Session, *, obj_in: MembershipGroupCreate) -> MembershipGroup:
    db_obj = MembershipGroup(**obj_in.model_dump())  # Pydantic v2
    db.add(db_obj)
    db.commit()
    db.refresh(db_obj)
    return db_obj
...
def update(
    db: Session,
    *,
    db_obj: MembershipGroup,
    obj_in: Union[MembershipGroupUpdate, Dict[str, Any]]
) -> MembershipGroup:
...
def remove(db: Session, *, id: int) -> Optional[MembershipGroup]:
    ...
    # The current context is an f-string being written to a file, so MembershipGroup is correct there.
    ...
    obj = db.query(MembershipGroup).filter(MembershipGroup.groupID == id).first()
    # A more robust get for single PKs would be:
    # obj = db.get(MembershipGroup, id) # if pk_type_hint is guaranteed to match actual PK type
