...
from app.models.tipo_actividad import TipoActividad # Use model_name_str for consistency
...
def get(db: Session, id: int) -> Optional[TipoActividad]:
    return db.query(TipoActividad).filter(TipoActividad.tipo_actividad_id == id).first()

def get_multi(db: Session, *, skip: int = 0, limit: int = 100) -> List[TipoActividad]:
    return db.query(TipoActividad).offset(skip).limit(limit).all()

def create(db: Session, *, obj_in: TipoActividadCreate) -> TipoActividad:
    db_obj = TipoActividad(**obj_in.model_dump())  # Pydantic v2
    db.add(db_obj)
    db.commit()
    db.refresh(db_obj)
    return db_obj
...
def update(
    db: Session,
    *,
    db_obj: TipoActividad,
    obj_in: Union[TipoActividadUpdate, Dict[str, Any]]
) -> TipoActividad:
...
def remove(db: Session, *, id: int) -> Optional[TipoActividad]:
    ...
    # The current context is an f-string being written to a file, so TipoActividad is correct there.
    ...
    obj = db.query(TipoActividad).filter(TipoActividad.tipo_actividad_id == id).first()
    # A more robust get for single PKs would be:
    # obj = db.get(TipoActividad, id) # if pk_type_hint is guaranteed to match actual PK type
