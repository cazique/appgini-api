...
from app.models.estado_noticia import EstadoNoticia # Use model_name_str for consistency
from app.schemas.estado_noticia import EstadoNoticiaCreate, EstadoNoticiaUpdate

# Note: get_multi might need more sophisticated filtering/ordering in a real app

def get(db: Session, id: int) -> Optional[EstadoNoticia]:
    return db.query(EstadoNoticia).filter(EstadoNoticia.estado_noticia_id == id).first()

def get_multi(db: Session, *, skip: int = 0, limit: int = 100) -> List[EstadoNoticia]:
    return db.query(EstadoNoticia).offset(skip).limit(limit).all()

def create(db: Session, *, obj_in: EstadoNoticiaCreate) -> EstadoNoticia:
    db_obj = EstadoNoticia(**obj_in.model_dump())  # Pydantic v2
    db.add(db_obj)
    db.commit()
    db.refresh(db_obj)
    return db_obj
...
def update(
    db: Session,
    *,
    db_obj: EstadoNoticia,
    obj_in: Union[EstadoNoticiaUpdate, Dict[str, Any]]
) -> EstadoNoticia:
...
def remove(db: Session, *, id: int) -> Optional[EstadoNoticia]:
    ...
    # The current context is an f-string being written to a file, so EstadoNoticia is correct there.
    ...
    obj = db.query(EstadoNoticia).filter(EstadoNoticia.estado_noticia_id == id).first()
    # A more robust get for single PKs would be:
    # obj = db.get(EstadoNoticia, id) # if pk_type_hint is guaranteed to match actual PK type
