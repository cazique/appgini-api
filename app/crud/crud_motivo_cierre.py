...
from app.models.motivo_cierre import MotivoCierre # Use model_name_str for consistency
from app.schemas.motivo_cierre import MotivoCierreCreate, MotivoCierreUpdate

# Note: get_multi might need more sophisticated filtering/ordering in a real app

def get(db: Session, id: int) -> Optional[MotivoCierre]:
    return db.query(MotivoCierre).filter(MotivoCierre.motivo_cierre_id == id).first()

def get_multi(db: Session, *, skip: int = 0, limit: int = 100) -> List[MotivoCierre]:
    return db.query(MotivoCierre).offset(skip).limit(limit).all()

def create(db: Session, *, obj_in: MotivoCierreCreate) -> MotivoCierre:
    db_obj = MotivoCierre(**obj_in.model_dump())  # Pydantic v2
    db.add(db_obj)
    db.commit()
    db.refresh(db_obj)
    return db_obj
...
def update(
    ...
    *,
    db_obj: MotivoCierre,
    obj_in: Union[MotivoCierreUpdate, Dict[str, Any]]
) -> MotivoCierre:
...
def remove(db: Session, *, id: int) -> Optional[MotivoCierre]:
    ...
    # The current context is an f-string being written to a file, so MotivoCierre is correct there.
    ...
    obj = db.query(MotivoCierre).filter(MotivoCierre.motivo_cierre_id == id).first()
    # A more robust get for single PKs would be:
    # obj = db.get(MotivoCierre, id) # if pk_type_hint is guaranteed to match actual PK type
