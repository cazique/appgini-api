...
from app.models.usuario import Usuario # Use model_name_str for consistency
from app.schemas.usuario import UsuarioCreate, UsuarioUpdate

# Note: get_multi might need more sophisticated filtering/ordering in a real app

def get(db: Session, id: str) -> Optional[Usuario]:
...
def create(db: Session, *, obj_in: UsuarioCreate) -> Usuario:
    db_obj = Usuario(**obj_in.model_dump())  # Pydantic v2
    db.add(db_obj)
    db.commit()
    db.refresh(db_obj)
    return db_obj
...
def remove(db: Session, *, id: str) -> Optional[Usuario]:
    ...
    # The current context is an f-string being written to a file, so Usuario is correct there.
    ...
    obj = db.query(Usuario).filter(Usuario.usuario_id == id).first()
    # A more robust get for single PKs would be:
    # obj = db.get(Usuario, id) # if pk_type_hint is guaranteed to match actual PK type
